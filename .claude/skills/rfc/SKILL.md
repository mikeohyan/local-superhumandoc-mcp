...
The process itself is specified by the `rfc-process` topic; `_rfc/README.md`
resolves that topic to its current RFC and path. This skill is the operating
...
is not generated by a script, by deliberate choice recorded by the `rfc-process`
topic. It carries one table per location (in flight, implemented, retired), each
