...
- [ ] **Step 5: Verify the real console script fails cleanly with no `.env`**
...
- [ ] `uv build --wheel` succeeds; a real stdio subprocess against the built
      console script still completes `initialize()` and returns zero tools,
      **with a valid `.env` present**.
- [ ] With no `.env` anywhere: exit code 2, one readable stderr line, no
      traceback.
- [ ] With `--env-file` naming a missing path while a valid `./.env` exists:
      exit 2, and the error names the missing path. This is the rule that
      prevents binding to the wrong document.
- [ ] `grep -r` the working tree for the real token value from `.env`: absent
      from all source, tests and output.
- [ ] Citation audit: no RFC number or path in `src/` or `tests/`.
