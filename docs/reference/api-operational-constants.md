...
| `EXPORT_DEADLINE_S` | `45.0` | [DECIDED — one measurement] | Owned by the `async-operations` topic, retuned 2026-09-07 under its rule that these values move against observation without superseding. Bounds how long this client waits for **one export** before reporting that it could not obtain the page — distinct from `TOOL_CALL_DEADLINE_S`, which bounds everything the call does. Probe E1 measured three exports end to end at 2.9, 3.1 and 6.6 s, so 45 s is roughly seven times the slowest observed and fires on nothing resembling normal behaviour; it sits well below the eighty-second working budget, so unlike its predecessor it *can* fire, and it leaves at least thirty-five seconds of the call for everything else. **It was previously 90.0 and inert**: equal to `TOOL_CALL_DEADLINE_S` while the `failure-policy` topic makes that the only authority and reserves a ten-second tail, a subordinate ceiling equal to the outer total can never be the one that fires. Three runs on two small pages is not a distribution and says nothing about a large document under load; this number is one measurement's worth of confidence, not a characterisation |
